...
<div class="admin-page">
    ...
    <div class="card admin-grid-wrap" style="flex:1 1 0%; min-height:0; display:flex; flex-direction:column; padding:0.35rem;">
        <div id="inquiry-grid" data-ax5grid="inquiry-grid" style="height:100%; min-height:240px;"></div>
    </div>
</div>

<script>
(function() {
var apiBase = '/admin/inquiries/api.php';
...
function loadList() {
    fetch(apiBase).then(function(res) {
        ...
        var search = (document.getElementById('search_title') || {}).value.trim().toLowerCase();
        if (search) list = list.filter(function(r) { return (r.title || '').toLowerCase().indexOf(search) >= 0; });
        gridData = list;
...
function initGrid() {
    ...
    gridInstance.setConfig({
        target: $('[data-ax5grid="inquiry-grid"]'),
        showLineNumber: true,
        ...
        columns: [
            ...
            { key: 'product_name', label: '상품명', width: 120, align: 'left' },
            { key: 'user_id', label: '회원ID', width: 80, align: 'right' },
            { key: 'title', label: '제목', width: 200, align: 'left' },
            { key: 'answered_at', label: '답변여부', width: 80, align: 'center' },
            { key: 'created_at', label: '문의일', width: 160, align: 'left' }
        ]
    });
