...
<div class="admin-page">
        ...
        <form id="form-search" onsubmit="return false;">
            <div class="form-row">
                <div class="form-field">
                    ...
                    <input type="text" id="search_content" placeholder="내용">
                </div>
            </div>
        </form>
    </div>
    ...
    <div class="card admin-grid-wrap" style="flex:1 1 0%; min-height:0; display:flex; flex-direction:column; padding:0.35rem;">
        <div id="review-grid" data-ax5grid="review-grid" style="height:100%; min-height:240px;"></div>
    </div>
</div>

<script>
(function() {
var apiBase = '/admin/reviews/api.php';
...
function loadList() {
    fetch(apiBase).then(function(res) {
        ...
        var search = (document.getElementById('search_content') || {}).value.trim().toLowerCase();
        if (search) list = list.filter(function(r) { return (r.content || '').toLowerCase().indexOf(search) >= 0; });
        gridData = list;
...
function initGrid() {
    ...
    gridInstance.setConfig({
        target: $('[data-ax5grid="review-grid"]'),
        showLineNumber: true,
        ...
        columns: [
            ...
            { key: 'product_name', label: '상품명', width: 140, align: 'left' },
            { key: 'user_id', label: '회원ID', width: 80, align: 'right' },
            { key: 'rating', label: '별점', width: 60, align: 'center' },
            { key: 'content', label: '내용', width: 220, align: 'left' },
            { key: 'created_at', label: '작성일', width: 160, align: 'left' }
        ]
    });
